...
function LoginPage() {
  ...
  const submit = async (event) => { event.preventDefault(); setError(""); try { const response = await fetch("/api/auth/login", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ username, password }) }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error || "Sign in failed"); window.localStorage.setItem("dfds-access-token", payload.accessToken); const target = new URLSearchParams(window.location.search).get("return_to") || "/it-data-flow"; window.location.assign(target); } catch (cause) { setError(cause.message); } };
  return <section className="login-shell">
    ...
    <main className="login-panel"><form onSubmit={submit} className="login-form-wrap"><p className="login-eyebrow">SECURE WORKSPACE</p><h2>Sign in</h2><p className="login-intro">Use your workspace credentials to continue.</p><label htmlFor="login-username">Username</label><input id="login-username" value={username} onChange={(event) => setUsername(event.target.value)} placeholder="Enter your username" autoComplete="username" required /><label htmlFor="login-password">Password</label><input id="login-password" type="password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Enter your password" autoComplete="current-password" required />{error ? <p className="login-error" role="status">{error}</p> : null}<button className="login-submit" type="submit">Sign in <span aria-hidden="true">→</span></button></form></main>
  </section>;
}
